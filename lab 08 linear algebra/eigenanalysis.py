...
def power_method(mat_a, epsilon=1e-9, b_verbose=False):
    ...
    while True:
        # 행렬 곱셈
        # k 가 큰 값이라면 z_k 는 첫번쨰 고유벡터와 거의 같은 방향이므로
        # y_k+1 = mat_a z_k = lambda_1 z_k
        # z_k 의 가장 큰 요소는 1 이었으므로
        # y_k+1의 가장 큰 요소가 lambda_1 인 것이락로 볼 수 있다
...
def cholesky_decomposition(mat_a):
    ...
    Symmetric Matrix
    :return:
    mat_i matrix such that mat_i mat_I_t == mat_a
    """
    """
    ...
    l_ki = (1/l_00) * (A_ki - sum([l_ij l_jk for j in range(0, i_1)]) ) for i in range(1, k - 1)
