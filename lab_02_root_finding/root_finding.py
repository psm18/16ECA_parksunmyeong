...
def bisection(f, xl, xh, epsilon=epsilon_global, b_verbose=False):
    ...
    f(xl)rhk f(xn)의 부호가 반대이면 xh를 xn으로 옮김
    ...
    while True:
        ...
        if f(xn) * f(xh) < 0:
            # 다르면 : 근이 xn ~ xh 사이에 있음. xl 에 xn을 저장
            xl = xn
        else:
...
def newton(f, df, x0, epsilon=epsilon_global, b_verbose=False):
    ...
    이를 이용하여 i + 1 번쨰 x를 xi - fi/di 로 정함

    f(x)의 xi 에서의 기울기 di 의 절대값이 0에 가까울 경우 xi 로 부터 매우 먼 위치에 xi 가 자리하게 됨
...
def func(x):
    ...
    이 경우는 x * x -2.0 = 0 을 만족하는 x 를 찾게 되며 이러한 x 는 2 ** 0.5 즉 2의 제곱근임
    :param x:
    :return:
    """
    return 1.0 * x * x -2.0
...
def main():
    ...
    x0 = "0.01" # x 의 초기값. 정수인가? 실수인가? 문자열인가?
