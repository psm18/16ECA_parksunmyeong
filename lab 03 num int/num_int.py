...
def simpson2(f, xi, xe, n=100, b_verbose=False):
    ...
    :param ex: 정젹분 끝 지점
    :param n: 구획을 나누는 갯수
    :param b_vervose: 함수 실행 중 상세 내용 표시 여부
    :return: f(x) 를 xi ~ xe 구간에서 정적분한 근사값
    """
    # 초기화 시작
    ...
    # 적분 구획 시작 지점 k 번쨰 x 초기화
    xk = xi
    # 적분 구획 시작 지점 함수값 k 번쨰 f(x) 초기화
    fxk = f(xk)
    ...
    for k in xrange(0, n, 2):
        ...
        # k 번째, k +1 번째 두 구획의 면적의 함을 simpson 공식으로 계산
...
def main():
    ...
    # 적분 결과를 그림으로 표시하기 위하여 pylab 모듈로 부터 기능을 불러 들임
