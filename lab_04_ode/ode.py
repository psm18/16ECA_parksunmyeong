...
def fwd_euler(f, x_init, t_start, t_end, delta_t):
    ...
    # list_x 를 초기화 함
    # list_x[0] == k =0 일떄의 x 값 == 초기값 x_init
    list_x = [tuple(x_init)]

    # 행 할당 반복문 시작
    #   list_x[0] 는 위에서 이미 초기값 x_init 로 정함
    #    k=[1, 2, ..., n-1] 인 경우 x 즉
    #       list_x[1], list_x[2], ..., list[n-1] 의 값은 아직 알 수 없음
    ...
    for k in list_t[1:]:
        ...
        # 다른 프로그래밍 언어의 경우 자료를 저장하기 전에 장소를 할당할 필요가 있을 경우가 많음
...
def mod_euler(f, x_init, t_start, t_end, delta_t):
    ...
    t[k] 와 t[k+1] 사이에는 dx/dt가 선형으로 변화할 것으로 가정함
    ...
    :return: 시간, x의 list
     ...
     # 각 time step 에서의 상태 변수 값 x와 시간 값 t 를 저장할 list 를 시작
     ...
     # time step 반복문 시작
     while tk1 < t_end:
         # x_list 에 마지막으로 추가된 요소로 k 번째 time step 의 상태 변수를 정함
         xk = x_list[-1]

         # step 1 : 전진 Euler 법과 같음. 이번 time step 에서의 기울기로 delta_t 만큼 전진
         sk = f(xk,tk)
         # xk 의 모든 요소 x 에 기울기 sk[i] 와 delta_t의 곱을 더한 결과를 list로 준비
         ...
         # step 3 : k번째 time step 과 k + 1 번째 time step 사이에는 기울기가 선형적으로 변화 했을 것으로 가정하여
...
def runge_while(f, x_init, t_init, t_end, delta_t):
    ...
    [0.0, 0.10000000000000001, 0.20000000000000001, 0.30000000000000004, 0.40000000000000002, 0.5]
    >>> print list_x
    [[0], [0.0076608912592762328], [0.029394418941171337], [0.06235025026746614], [0.1026147916147\61461737], [0.14559255884915154]]
    """

    # 각 time step 에서의 상태 변수 값 x 와 시간 값 t 를 저장할 list를 시작
    # 한 time step 마다 하나씩 덧붙임
    ...
    # 가중 평균 계산을 위해 6으로 나누기 떄문데 계수를 미리 준비
    delta_t_sixth = delta_t / 6.0

    # k + 1/2 번째 time step 의 시간 tk 초기화
    tk = t_init
    # k + 1/2번째 time step 의 시간 tk_half 초기화
    ...
    # time step 반복문 시작
    while tk1 < t_end:
        # x_list 에 마지막으로 추가된 요소로 k번쨰 time step의 상태 변수를 정함
        xk = x_list[-1]

        #step 1 : 전진 Euler 법과 같음. k 번째 time step 에서의 기울기 k1
        k1 = f(xk, tk)

        # step 2: 위에서 계산한 기울기로 k + 1/2 번쨰 time step 의 상태 변수를 추정하여 기울기 k2를 계산
        xk1_p = [(xk[i] + k * delta_t_half) for (i, k) in enumerate(k1)]
        k2 = f(xk1_p, tk_half)

        # step 3 : 위에서 계산한 기울기로 k + 1/2 번쨰 time step 의 상태 변수를 추정하여 기울기 k2를 계산
        xk2_p = [(xk[i] + k * delta_t_half) for (i, k) in enumerate(k2)]
        k3 = f(xk2_p, tk_half)

        # step 4 : 기울기 k3 로 k + 1 번쨰 time step 의 상태 변수를 추정하여 기울기 k4를 계산
